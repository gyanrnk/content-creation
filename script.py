...
def _build_messages(topic: str, mode: str, num_segments: int, context: str = ""):
    ...
    user = (
        ...
        "- Segment 1 = a SCROLL-STOPPING HOOK in its FIRST few words: a bold/shocking "
        "claim or a curiosity-gap question that PROMISES a payoff. NO intro, NO 'aaj "
        "hum baat karenge', NO greeting — jump straight into the hook. This one line "
...
def _build_meta_messages(topic: str, mode: str, segments: list, context: str = ""):
    ...
    user = (
        ...
        "- title_options = 3 DIFFERENT catchy click-worthy titles (different angles: "
        "one curiosity/question, one bold claim, one listicle/number). Each under "
        "70 chars, Hindi+English mix ok, 1-2 emojis. SEO: include the main "
        "searchable keywords people would type (player/team/'World Cup 2026').\n"
        "- youtube_title = the strongest of title_options.\n"
        "- description = 3-5 sentence engaging HINGLISH caption (jaise dost se baat). "
        f"{pred}End by asking viewers to comment (e.g. 'Tumhe kya lagta hai? Comment me batao').\n"
        "- hashtags = 8-10 hashtags one string: MIX of broad (#Shorts #Football "
        "#Reels #viral) + niche/specific (player/team/#WorldCup2026) for max discovery.\n"
        "Output JSON only. /no_think"
    )
    return system, user
