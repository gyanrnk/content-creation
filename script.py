...
MODE_GUIDE = {
    ...
    "ranking": (
        ...
        "*** EACH item must be COMPLETE, not a headline: (a) WHAT happened, AND (b) the "
        "DRAMA that made it unforgettable — the score, the minute, the opponent, what was "
        "at stake, or the emotion. Bad (incomplete): 'He almost lost the 2022 final but "
        "still won the cup.' Good: 'France ne 3-3 se wapsi ki aur Mbappe ne hat-trick "
        "maari, par Messi ne penalty shootout me 36 saal ka intezaar khatam kiya.' *** "
        "The #1 item is the BIGGEST — give it the most dramatic treatment, never a flat "
        "one-liner. *** BANNED lazy reasons: 'his skills are praised', 'is very good', "
...
def _build_messages(topic: str, mode: str, num_segments: int, context: str = ""):
    ...
    user = (
        ...
        "*** WORD COUNT — STRICT BAND: every voice_english must be 17 to 22 words. "
        "COUNT them. Under 17 words = INCOMPLETE (a bare headline with no drama/detail — "
        "the #1 complaint about our videos). Over 22 words = a RUN-ON (video too long). "
        "Voice is played at +50%% speed, so 22 words is still fast — USE the room to make "
        "each line COMPLETE. Use EXACTLY ONE connector "
