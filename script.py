...
MODE_GUIDE = {
    ...
    "crossover": (
        "ONE FOOTBALL STAR is the HERO — the video answers: which stars from OTHER "
        "sports have praised HIM, and what exactly did they say. The outside-sport "
        "stars are the guests; the footballer is the subject of every line.\n"
        "*** SEGMENT 1 MUST SET THE SCENE with the biggest admirer: who (name + his "
        "sport), where he said it (interview / tweet / podcast / courtside), and that "
        "it is about THE footballer. ***\n"
        "*** Each following segment = a DIFFERENT admirer (ideally a different sport) "
        "praising the SAME footballer. Escalate: good praise -> bigger -> the most "
        "jaw-dropping line last. NEVER drift into what the admirer said about anyone "
        "else — if Djokovic praised Ronaldo AND Brady, only the Ronaldo part belongs "
        "here. And NEVER the reverse direction: the footballer praising someone else "
        "does NOT belong in this video at all. ***\n"
    ...
    "ranking": (
        ...
        "Tease that #1 will shock them in the hook. "
        "*** DO NOT waste words saying 'At number five' / 'For number four' — the rank "
        "number is ALREADY shown on screen, and repeating that phrasing every line is "
        "boring. Start each line with the MOMENT itself and open each one differently. *** "
        "*** Each item = ONE striking detail, said SHORT. Not a summary, not a list of "
        "everything — pick the single most shocking number/moment and drop it. "
        "Weak: 'He is a great player and won many trophies.' "
        "Strong: 'Mbappe ki hat-trick bhi Messi ko nahi rok paayi.' "
        "Stay inside the word limit — long lines kill the video. *** "
        "*** BANNED lazy reasons: 'his skills are praised', 'is very good', "
        "'is talented', 'is amazing' — these say NOTHING. *** "
...
def _build_messages(topic: str, mode: str, num_segments: int, context: str = ""):
    ...
    user = (
        ...
        "*** The VERIFIED FACTS may contain SEVERAL unrelated stories. PICK EXACTLY ONE "
        "and ignore the rest. A script that jumps (Trump's delay -> Messi's record -> "
        "the halftime show -> the final) is WRONG — it feels empty because nothing is "
        "developed. Four lines on ONE event beats four lines on four events. ***\n"
        "*** CHAIN RULE — this is what makes a script feel connected instead of a list: "
        "every segment AFTER the first must OPEN by linking back to the line before it. "
        "Test: if you read segment 3 on its own and it makes complete sense with no "
        "memory of segment 2, it is WRONG — rewrite it. Order the segments so they "
        "ESCALATE (oldest to newest, or bad to worse, or small to biggest) so the "
        ...
        "in minute 92, so he held his shirt up.' — the viewer feels no build-up and swipes "
        "away. RIGHT (same facts, chained + escalating): 'In 2000 Figo left Barca for "
        "Madrid — the ultimate betrayal.' / 'Two years later fans answered: a severed pig "
        "head thrown at his corner flag.' / 'But the players got uglier than the fans — "
        "Mourinho poked Vilanova in the eye.' ***\n"
        "*** NEVER make a vague claim — always give its SPECIFIC number/name in the SAME "
        "line. BANNED: 'set a new record', 'won many trophies', 'made history', 'shocked "
        "everyone' with no detail. Write 'apna 9th World Cup goal maara — Pele ka record "
        "toda', NOT 'set a new record'. If the facts do not give you the specific number, "
        "choose a different fact that does. THIS is what makes a short feel complete. ***\n"
        ...
        "*** WORD COUNT: every voice_english must be 13 to 16 words. "
        "COUNT them. Under 13 = thin/empty (viewers ignore it); over 16 = the video gets "
        "too long. Inside that budget pack REAL DETAIL — a name, a year, a number, "
        "a place. Use EXACTLY ONE connector "
        ...
        "0. MEASURED ON THIS CHANNEL (retention curves): every flop loses HALF its "
        "viewers between 10-30%% of the video — i.e. the FIRST CONTENT LINE. So LINE 1 "
        "IS THE PAYLOAD: it must contain the single most shocking CONCRETE fact of the "
        "whole story (the number, the record, the thing that happened). If a viewer "
        "heard ONLY line 1, they should already know the wildest part. NEVER open with "
        "setup ('Ek ladka tha jo...', 'Football me ek aisa club he...'). Setup is where "
        "our viewers swipe. ***\n"
        "0b. NUMBERED SET = RETENTION MACHINE (measured: our two best-retained videos, "
        "94.8%% and 169.9%% avg viewed, are BOTH '5 things' countdowns — viewers stay to "
        "complete the set). Whenever the story allows, frame it as a NUMBERED "
        "progression and SAY the numbers in the lines ('Pehla...', 'Doosra...', "
        "'Aur number one...'). Save the biggest item for LAST. ***\n"
        ...
        "LATE is what keeps them watching — but the GAP must still be concrete (rule 0), "
        "not vague setup.\n"
